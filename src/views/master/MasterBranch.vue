<template>
  <AppLayout>
    <div class="page-header">
      <div>
        <h1>Master Branch</h1>
        <p>Pull master branch data by date range</p>
      </div>
    ...
    <div v-if="showConfirmDialog" class="modal-overlay" @click="closeConfirmDialog">
      <div class="confirm-modal" @click.stop>
        ...
        <p>Are you sure you want to pull master branch data?</p>
        <p class="info-text">This will fetch the latest branch data from the source.</p>
          ...
          <div class="transactions-table-wrapper">
            <div class="table-header">
              ...
              <div class="search-box">
                ...
                <input type="text" v-model="searchDetail" placeholder="Search branches...">
              </div>
            </div>
              ...
              <table class="transactions-table">
                ...
                <tbody>
                  <tr v-for="detail in filteredDetails" :key="detail.id">
                    <td>{{ detail.branchNo || '-' }}</td>
                    <td class="item-name">{{ detail.branchName || '-' }}</td>
                    <td class="address-cell">{{ detail.address || '-' }}</td>
                    <td class="actions">
...
const loadGroups = async () => {
  ...
  try {
    const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/master-branch/groups`, {
      params: {
...
const confirmFetchData = async () => {
  ...
  try {
    const response = await axios.post(
      `${import.meta.env.VITE_API_URL}/api/master-branch/fetch`,
      {},
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
...
const viewGroupDetail = (group) => {
  router.push(`/master-branch/group/${group.id}`)
}
...
const filteredDetails = computed(() => {
  ...
  return selectedGroup.value.details.filter(detail => 
    (detail.branchNo && detail.branchNo.toLowerCase().includes(query)) ||
    (detail.branchName && detail.branchName.toLowerCase().includes(query)) ||
    (detail.address && detail.address.toLowerCase().includes(query))
  )
})

const deleteDetail = async (detail) => {
  if (!confirm(`Hapus branch ${detail.branchName || 'ini'}?`)) return
  
  try {
    const response = await axios.delete(
      `${import.meta.env.VITE_API_URL}/api/master-branch/details/${detail.id}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
...
const deleteGroup = async (group) => {
  ...
  try {
    const response = await axios.delete(
      `${import.meta.env.VITE_API_URL}/api/master-branch/groups/${group.id}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
